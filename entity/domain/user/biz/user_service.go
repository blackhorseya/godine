...
type IUserBiz interface {
	...
	Login(ctx contextx.Contextx) (item *model.Account, err error)
	...
	GetUser(ctx contextx.Contextx, id string) (item *model.Account, err error)

	// ListUsers retrieves a list of users based on the provided options.
	ListUsers(ctx contextx.Contextx, options ListUsersOptions) (items []*model.Account, total int, err error)
